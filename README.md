...
| Terminal Chart | `futures_usds.klineCandlestickData` | Live candlestick feed with EMA overlays |
...
**News/sentiment layer (optional booster, non-blocking):** real headlines are pulled live from Cointelegraph's RSS feed and scored (Bullish/Bearish/Neutral + confidence) — the AI only classifies sentiment, it never writes or invents headline text. If sentiment agrees with the trade direction, it adds one extra point to the confluence checklist — but a trade already meeting the technical requirement is never blocked by absent or neutral news.
...
Judges/users can optionally connect their own Binance Agent OS session from the **Connect Account** tab:

- Paste your own MCP session token/API key, or use **Demo Mode** to skip this entirely.
...
- **Live Trading is off by default** ("Simulated") — a separate "Enable Live" action is required to allow real order placement.
- Disconnecting or closing the tab clears the credential immediately.
...
The **Historical Backtest** panel runs the exact same confluence rules against 1,000 hourly candles across BTC, ETH, and SOL perpetuals and reports:
...
**Result on the last 1,000-hour window:** 100 trades, 38% win rate, +0.06R average, profit factor 1.04, -20.88% max drawdown, +3.7% total return.

This is a thin, positive edge — presented honestly as a starting point for live paper validation, not a guaranteed-profitable system. It's clearly labeled **"Historical Backtest — Simulated, Not Live"** and is never mixed with live paper trading data.

## Terminal Chart

A live candlestick view per asset (1m–1d intervals) with EMA 20/50 overlays, sourced directly from `futures_usds.klineCandlestickData`, alongside a manual order ticket for reviewing entries.
